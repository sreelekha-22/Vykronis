...
A GraalVM **`native`** job (Tier 2) is wired the same way: **Actions → CI → Run workflow → `run-native=true`** builds native binaries via the Spring Boot buildpack (16 GB runner; the 8 GB host laptop can't run `native-image`) for the **matrix** `policy-service` + `api-gateway` into `vykronis/<svc>:native`, pushes them to GHCR (`ghcr.io/sreelekha-22/vykronis/<svc>:native`), runs each, and asserts `/actuator/health` is `UP` — also reporting image size, boot time, and an approximate container RSS into the job summary and a `native-report.txt` artifact. Only the matrix services carry the `native` Maven profile today; AOT hint generation is verified locally via `./mvnw -pl platform/<svc> -Pnative spring-boot:process-aot` + an AOT JVM boot. See `docs/NATIVE.md`.
