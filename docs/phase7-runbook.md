...
A GraalVM **`native`** job (Tier 2 pilot) is wired the same way: **Actions → CI → Run workflow → `run-native=true`** builds the `policy-service` native binary via the Spring Boot buildpack (16 GB runner; the 8 GB host laptop can't run `native-image`) into `vykronis/policy-service:native`, runs it, and asserts `/actuator/health` is `UP`. Only `policy-service` carries the `native` Maven profile today; AOT hint generation is verified locally via `./mvnw -pl platform/policy-service -Pnative spring-boot:process-aot`.
